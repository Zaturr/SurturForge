...
func runBenchmark(pattern, desc string) (string, *cpu.CPUStats, error) {
	...
	// Importante: NO usamos `go test -bench` aquí porque el binario puede
	// ejecutarse sin el repo (sin go.mod / sin el código fuente).
	outputStr, err := cpu.RunGoStyleBenchmarks(pattern, 3*time.Second)
	close(done)
